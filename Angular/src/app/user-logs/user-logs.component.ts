...
//  SELECT ARE CASE SENSITIVE! 
 ...
 odataParams = `$top=9999&$Select=*`;// THIS IS WORKING
...
*/
    ...
    if (this.odataParams.toString().indexOf("$filter") == -1){
      this.odataParams = this.odataParams + `&$filter=CreatedBy eq 0`;
    }
   
    //console.log(`loadRecords odataParams: ${this.odataParams} - this.odataParams.toString().indexOf("$filter"): ${this.odataParams.toString().indexOf("$filter")}`);
